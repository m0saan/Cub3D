...
int mapito(t_struct *data, int *pos, char *buff)
{
	...
	while (buff[*pos] != '\0' && i < data->n_lines)
	{
		...
		{
			buff[*pos] == ' ' ? *pos += 1 : *pos;
			if (buff[*pos] != '1' && buff[*pos] != '0' && buff[*pos] != '2' && buff[*pos] != 'N' && buff[*pos] != 'W' && buff[*pos] != 'E' && buff[*pos] != 'S')
				return 0;
...
int get_resolution_und_floor_values(t_struct *data, char *buff, int *get_to_map, int *pos)
{
	if (buff[*pos] == 'R')
	{
		pos++;
...
int		get_texture_path(t_struct *data, char *buff, int *get_to_map, int *pos)
{
...
int read_map(t_struct *data, char *buff)
{
	...
	while (buff[pos])
	{
		...
		if (buff[pos] == '1' && buff[pos + 1] == ' ' && buff[pos + 2] == '1' && get_to_map == 7)
		{
