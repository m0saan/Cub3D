...
void set_up_sprite(t_struct *data)
{
	...
	while (i_spt < data->count_spt)
	{
		...
		spt_angle = atan2f(data->sprite[i_spt].y - data->y, data->sprite[i_spt].x - data->x);
		while (spt_angle - data->rotation_angle > M_PI)
			spt_angle -= 2 * M_PI;
		while (spt_angle - data->rotation_angle < -M_PI)
			spt_angle += 2 * M_PI;
		if (data->w_height > data->w_width)
			data->sprite[i_spt].size = ((float )data->w_height / data->sprite[i_spt].dis) * SQUARE_SIZE;
		else
			data->sprite[i_spt].size = ((float )data->w_width / data->sprite[i_spt].dis) * SQUARE_SIZE;
		data->sprite[i_spt].y_off = (float )data->w_height / 2 - data->sprite[i_spt].size / 2;
		data->sprite[i_spt].x_off = ((DEG(spt_angle) - DEG(data->rotation_angle)) * data->w_width) / SPRITE_SIZE + (((float )data->w_width / 2) - (data->sprite[i_spt].size / 2));
